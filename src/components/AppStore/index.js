...
class AppStore extends Component {
  state = {
    searchInput: '',
    userdetailsList: appsList,
    activeTabId: tabsList[0].tabId,
  }

  tabClicked = tabId => {
    this.steState({activeTabId: tabId})
  }

  onChange = event => {
    this.steState({searchInput: event.target.value})
  }

  render() {
    const {searchInput, userdetailsList, activeTabId} = this.state
    const searchResults = userdetailsList.filter(eachItem =>
      eachItem.appName
        .toLocaleLowerCase()
        .includes(
          searchInput.toLocaleLowerCase() &&
            eachItem.category.toLocaleLowerCase() ===
              activeTabId.toLocaleLowerCase(),
        ),
    )
